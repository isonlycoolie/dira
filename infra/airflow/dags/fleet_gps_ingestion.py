...
def run_fleet_gps_ingestion(batch_size: int = 50) -> dict[str, int]:
    simulator = FleetGPSSimulator()

    def fetcher(api_url: str, api_key: str) -> list[dict[str, Any]]:
        _ = api_url, api_key
    ...
    try:
        published = connector.ingest_from_api("mock://fleet-gps", "mock-api-key")
        return {"generated": batch_size, "published": published}
    finally:
        connector.disconnect()


with DAG(
    dag_id="fleet_gps_ingestion",
    start_date=datetime(2024, 1, 1, tzinfo=timezone.utc),
    schedule_interval=timedelta(hours=1),
    catchup=False,
    tags=["ingestion", "fleet"],
) as dag:
    ingest_fleet_gps_batch = PythonOperator(
        task_id="ingest_fleet_gps_batch",
        python_callable=run_fleet_gps_ingestion,
        dag=dag,
        retries=3,
